...
namespace AnvilPacker.Util
{
    ...
    public class DictionarySlim<TKey, TValue> : IReadOnlyCollection<KeyValuePair<TKey, TValue>> where TKey : IEquatable<TKey>
    {
        ...
        /// <summary>
        /// Gets the value for the specified key, or, if the key is not present,
        /// adds an entry, initializes it to <paramref name="initialValue"/>, then 
        /// returns the value by ref. This makes it possible to add or update a 
        /// value in a single look up operation.
        /// </summary>
        /// <param name="key">Key to look for</param>
        /// <returns>Reference to the new or existing value</returns>
        public ref TValue GetOrAdd(TKey key, TValue initialValue)
        {
            if (key == null) ThrowHelper.ThrowKeyArgumentNullException();
            Entry[] entries = _entries;
            int bucketIndex = key.GetHashCode() & (_buckets.Length - 1);
            ...
            ref TValue ptr = ref AddKey(key, bucketIndex);
            ptr = initialValue;
            return ref ptr;
        }
        public void Add(TKey key, TValue value)
        {
            ...
            for (int i = _buckets[bucketIndex] - 1; (uint)i < (uint)entries.Length; i = entries[i].next) {
                if (key.Equals(entries[i].key)) {
                    ThrowHelper.ThrowDuplicatedKeyException(key);
                }
            }
        ...
        private Entry[] Resize()
        {
            ...
            if ((uint)newSize > (uint)int.MaxValue) // uint cast handles overflow
                throw new InvalidOperationException("Hashtable's capacity overflowed and went negative. Check load factor, capacity and the current size of the table.");
